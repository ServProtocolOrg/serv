...
HTTPS_GIT := https://github.com/servprotocolorg/serv.git
...
build-linux:
	GOOS=linux GOARCH=amd64 LEDGER_ENABLED=false CGO_ENABLED=1 $(MAKE) build
...
build-docker:
	...
	$(DOCKER) create --name serv -t -i ${DOCKER_IMAGE}:latest serv
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp serv:/usr/bin/servnode ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/servprotocolorg/serv"
	godoc -http=:6060
...
localnet-start: localnet-stop
	@if ! [ -f build/node0/$(APP_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/serv:Z serv/node "./servnode testnet init-files --v 4 -o /serv --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\servnode:/serv\Z serv/node "./servnode tendermint unsafe-reset-all --home=/serv"
	@docker run --rm -v $(CURDIR)\build\node1\servnode:/serv\Z serv/node "./servnode tendermint unsafe-reset-all --home=/serv"
	@docker run --rm -v $(CURDIR)\build\node2\servnode:/serv\Z serv/node "./servnode tendermint unsafe-reset-all --home=/serv"
	@docker run --rm -v $(CURDIR)\build\node3\servnode:/serv\Z serv/node "./servnode tendermint unsafe-reset-all --home=/serv"
else
	@docker run --rm -v $(CURDIR)/build/node0/servnode:/serv:Z serv/node "./servnode tendermint unsafe-reset-all --home=/serv"
	@docker run --rm -v $(CURDIR)/build/node1/servnode:/serv:Z serv/node "./servnode tendermint unsafe-reset-all --home=/serv"
	@docker run --rm -v $(CURDIR)/build/node2/servnode:/serv:Z serv/node "./servnode tendermint unsafe-reset-all --home=/serv"
	@docker run --rm -v $(CURDIR)/build/node3/servnode:/serv:Z serv/node "./servnode tendermint unsafe-reset-all --home=/serv"
endif
...
PACKAGE_NAME:=github.com/servprotocolorg/serv
