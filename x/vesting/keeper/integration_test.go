...
import (
	...
	"github.com/servprotocolorg/serv/v12/x/vesting/types"
)
...
var _ = Describe("Clawback Vesting Account - Barberry bug", func() {
	var (
		...
		coinsNoNegAmount = sdk.Coins{
			sdk.Coin{Denom: constants.BaseDenom, Amount: sdk.NewInt(1e18)},
		}
		...
		coinsWithZeroAmount = sdk.Coins{
			sdk.Coin{Denom: constants.BaseDenom, Amount: sdk.NewInt(1e18)},
			sdk.Coin{Denom: constants.BaseDenom, Amount: sdk.NewInt(0)},
		}
		...
		for _, tc := range testcases {
			...
			It(tc.name, func() {
				...
				if tc.expError {
					...
					balance := s.app.BankKeeper.GetBalance(s.ctx, newAddr, constants.BaseDenom)
