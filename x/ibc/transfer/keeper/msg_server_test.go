...
import (
	...
	"github.com/servprotocolorg/serv/v12/x/ibc/transfer/keeper"
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() *types.MsgTransfer {
				...
				transferMsg := types.NewMsgTransfer("transfer", "channel-0", sdk.NewCoin(constants.BaseDenom, sdk.NewInt(10)), senderAcc.String(), "", timeoutHeight, 0, "")

				coins := sdk.NewCoins(sdk.NewCoin(constants.BaseDenom, sdk.NewInt(10)))
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.app.TransferKeeper = keeper.NewKeeper(
				...
				&MockICS4Wrapper{}, // No ICS4 wrapper
