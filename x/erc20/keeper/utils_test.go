...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	s.app = suite.ServChain.App.(*app.Serv)
	evmParams := s.app.EvmKeeper.GetParams(s.ServChain.GetContext())
	evmParams.EvmDenom = constants.BaseDenom
	err := s.app.EvmKeeper.SetParams(s.ServChain.GetContext(), evmParams)
	suite.Require().NoError(err)

	// s.app.FeeMarketKeeper.SetBaseFee(s.ServChain.GetContext(), big.NewInt(1))

	// Set block proposer once, so its carried over on the ibc-go-testing suite
	validators := s.app.StakingKeeper.GetValidators(suite.ServChain.GetContext(), 2)
	cons, err := validators[0].GetConsAddr()
	suite.Require().NoError(err)
	suite.ServChain.CurrentHeader.ProposerAddress = cons.Bytes()

	err = s.app.StakingKeeper.SetValidatorByConsAddr(suite.ServChain.GetContext(), validators[0])
	suite.Require().NoError(err)

	_, err = s.app.EvmKeeper.GetCoinbaseAddress(suite.ServChain.GetContext(), sdk.ConsAddress(suite.ServChain.CurrentHeader.ProposerAddress))
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(suite.ServChain.GetContext(), minttypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(suite.ServChain.GetContext(), minttypes.ModuleName, suite.ServChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(s.ServChain.GetContext(), types.ModuleName, coins)
	...
	err = s.app.ClaimsKeeper.SetParams(suite.ServChain.GetContext(), claimparams)
	suite.Require().NoError(err)

	params := types.DefaultParams()
	params.EnableErc20 = true
	err = s.app.Erc20Keeper.SetParams(suite.ServChain.GetContext(), params)
	suite.Require().NoError(err)
	...
	coins = sdk.NewCoins(nativeCoin)
	err = s.app.BankKeeper.MintCoins(suite.ServChain.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(suite.ServChain.GetContext(), types.ModuleName, authtypes.FeeCollectorName, coins)
	suite.Require().NoError(err)
}
...
// to the Serv chain (used on IBC tests)
func (suite *KeeperTestSuite) DeployContractToChain(name, symbol string, decimals uint8) (common.Address, error) {
	return testutil.DeployContract(
		s.ServChain.GetContext(),
		s.ServChain.App.(*app.Serv),
		suite.ServChain.SenderPrivKey,
		suite.queryClientEvm,
		contracts.ERC20MinterBurnerDecimalsContract,
		name, symbol, decimals,
	)
