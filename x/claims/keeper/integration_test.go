...
var _ = Describe("Claiming", Ordered, func() {
	...
	// https://github.com/servprotocolorg/serv/issues/41
	...
	totalClaimsAmount := sdk.NewCoin(constants.BaseDenom, claimValue.MulRaw(int64(accountCount)))
	...
	delegateAmount := sdk.NewCoin(constants.BaseDenom, sdk.NewInt(1))
	initBalance := sdk.NewCoins(
		sdk.NewCoin(constants.BaseDenom, initClaimsAmount.Add(initBalanceAmount)), // claimsDenom == evmDenom
	)
	...
	initBalance0 := sdk.NewCoins(
		...
		sdk.NewCoin(constants.BaseDenom, initBalanceAmount.Add(initClaimsAmount0)), // claimsDenom == evmDenom
	)
	...
	BeforeAll(func() {
		...
		poolBalance := s.app.BankKeeper.GetBalance(s.ctx, distrAddr, constants.BaseDenom)
		Expect(poolBalance.IsZero()).To(BeTrue())

		// ensure module account has the escrow fund
		balanceClaims := s.app.BankKeeper.GetBalance(s.ctx, claimsAddr, constants.BaseDenom)
	...
	Context("before decay duration", func() {
		...
		BeforeAll(func() {
			...
			initialPoolBalance = s.app.BankKeeper.GetBalance(s.ctx, distrAddr, constants.BaseDenom)
			actionV = sdk.NewCoin(constants.BaseDenom, actionValue)
		})

		It("can claim ActionDelegate", func() {
			addr := getAddr(privs[0])
			prebalance := s.app.BankKeeper.GetBalance(s.ctx, addr, constants.BaseDenom)
		...
		It("did not clawback to the community pool", func() {
			...
			balanceClaims := s.app.BankKeeper.GetBalance(s.ctx, claimsAddr, constants.BaseDenom)
			totalClaimed = sdk.NewCoin(constants.BaseDenom, actionV.Amount.MulRaw(3))
	...
	Context("at 2/3 decay duration", func() {
		...
		BeforeAll(func() {
			actionV = sdk.NewCoin(constants.BaseDenom, actionValue.QuoRaw(3))
			unclaimedV = sdk.NewCoin(constants.BaseDenom, actionValue.Sub(actionV.Amount))
		...
		It("did not clawback to the community pool", func() {
			remainderUnclaimed = sdk.NewCoin(constants.BaseDenom, unclaimedV.Amount.MulRaw(4))
			totalClaimed = totalClaimed.Add(sdk.NewCoin(constants.BaseDenom, actionV.Amount.MulRaw(4)))
	...
	Context("after decay duration", func() {
		BeforeAll(func() {
			...
			Expect(moduleBalances.AmountOf(constants.BaseDenom)).To(Equal(totalClaimsAmount.Sub(totalClaimed).Sub(remainderUnclaimed).Amount))
		...
		It("cannot clawback already claimed actions", func() {
			addr := getAddr(privs[0])
			finalBalance := s.app.BankKeeper.GetBalance(s.ctx, addr, constants.BaseDenom)
		...
		It("can clawback unclaimed", func() {
			...
			Expect(moduleBalance.AmountOf(constants.BaseDenom).IsZero()).To(BeTrue())
		})
	})
})
