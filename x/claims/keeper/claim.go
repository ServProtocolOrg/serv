...
func (k Keeper) MergeClaimsRecords(
	...
	for _, action := range actions {
		...
		//  - the sender is not this chain's address and can't claim vote, delegation or evm actions
		...
		if senderClaimsRecord.HasClaimedAction(action) {
			return types.ClaimsRecord{}, errorsmod.Wrapf(errortypes.ErrNotSupported, "non-%s sender must not have claimed action: %v", constants.ApplicationName, action)
		}
