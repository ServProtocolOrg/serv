	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				coins := sdk.Coins{sdk.NewCoin(constants.BaseDenom, sdk.NewInt(100))}
		...
		},
		{
			...
			func() {
				...
				coins := sdk.Coins{sdk.NewCoin(constants.BaseDenom, sdk.NewInt(2))}
				// update the escrowed account balance to maintain the invariant
				err := testutil.FundModuleAccount(suite.ctx, suite.app.BankKeeper, types.ModuleName, coins)
				suite.Require().NoError(err)
				suite.app.Commit()
		...
		},
		{
			...
			func() {
				...
				coins := sdk.Coins{sdk.NewCoin(constants.BaseDenom, sdk.NewInt(200))}
				// update the escrowed account balance to maintain the invariant
				err := testutil.FundModuleAccount(suite.ctx, suite.app.BankKeeper, types.ModuleName, coins)
				suite.Require().NoError(err)
				suite.app.Commit()
