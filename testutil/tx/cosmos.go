...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'serv_80808-1'
	ChainID string
...
func PrepareCosmosTx(
	...
	if args.GasPrice != nil {
		fees = sdk.Coins{{Denom: constants.BaseDenom, Amount: args.GasPrice.MulRaw(int64(args.Gas))}}
	} else {
...
func signCosmosTx(
	...
	seq, err := chainApp.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := chainApp.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
