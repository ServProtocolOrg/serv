...
init_func() {
    ...
    "$PWD"/build/"$BINARY" validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/"$BINARY" start --pruning=nothing --rpc.unsafe \
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test node $HOST_RPC ..."
...
stop_func() {
    APP_PID=$i
    echo "shutting down node, pid=$APP_PID ..."
