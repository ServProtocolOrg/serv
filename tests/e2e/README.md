...
as it allows for initializing multiple Serv chains with different versions.
...
# previous node state folder. By default this is './build/.serv:/root/.serv'
...
# By default this is 'serv_80808-1'
CHAIN_ID
```
...
the test suite exports `/.serv` folder from the docker container
...
This package is decoupled from depending on the Serv codebase.
...
will be pulled from [DockerHub](https://hub.docker.com/r/servprotocolorg/serv/tags).
If it is not specified,
the test suite retrieves the second-to-last upgrade version
from the local codebase (in the `serv/app/upgrades` folder)
...
```log
ok  	github.com/servprotocolorg/serv/v12/tests/e2e	174.137s.
```
...
Error:  Received unexpected error:
        ...
        main.appCreator.newApp({{{0x3399b40, 0xc000ec1db8}, {0x33ac0f8, 0xc00...
            github.com/servprotocolorg/serv/v12/cmd/servnode/root.go:243 +0x2ca
...
```log
CONTAINER ID   IMAGE
9307f5485323   serv:local    <-- upgraded node
f41c97d6ca21   serv:v12.0.0   <-- initial node
```
