...
import (
	...
	"github.com/servprotocolorg/serv/v12/tests/e2e/upgrade"
)
...
func TestIntegrationTestSuite(t *testing.T) {
	t.Skip("FIXME: investigate about this test")
...
// runInitialNode builds a docker image capable of running a node with the given version.
...
func (s *IntegrationTestSuite) runInitialNode(version upgrade.VersionConfig) {
	...
	s.Require().NoError(err, "can't build container with %s version: %s", constants.ApplicationName, version.ImageTag)
	...
	s.Require().NoError(err, "can't run node with %s version: %s", constants.ApplicationName, version)
...
func (s *IntegrationTestSuite) runNodeWithCurrentChanges() {
	...
	err = s.upgradeManager.RunNode(node)
	s.Require().NoError(err, "can't run node %s using branch %s", constants.ApplicationName, branch)
}
...
func (s *IntegrationTestSuite) proposeUpgrade(name, target string) {
	...
	// if Serv is based on Evmos lower than v10.x.x no need to use the legacy proposal
	...
	s.Require().NoErrorf(
		err,
		"can't create the proposal to upgrade %s to %s at height %d with name %s",
		constants.ApplicationName, target, s.upgradeManager.UpgradeHeight, name,
	)

	outBuf, errBuf, err := s.upgradeManager.RunExec(ctx, exec)
	s.Require().NoErrorf(
		err,
		"failed to submit proposal to upgrade %s to %s at height %d\nstdout: %s,\nstderr: %s",
		constants.ApplicationName, target, s.upgradeManager.UpgradeHeight, outBuf.String(), errBuf.String(),
	)
...
func (s *IntegrationTestSuite) voteForProposal(id int) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	exec, err := s.upgradeManager.CreateVoteProposalExec(s.upgradeParams.ChainID, id, "--fees=500"+constants.BaseDenom, "--gas=500000")
...
func (s *IntegrationTestSuite) upgrade(targetRepo, targetVersion string) {
	...
	// export node home to local build/
	...
	node.SetCmd([]string{constants.ApplicationBinaryName, "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID)})
	err = s.upgradeManager.RunNode(node)
