...
import (
	...
	anteutils "github.com/servprotocolorg/serv/v12/app/ante/utils"
...
func (suite *AnteTestSuite) TestClaimStakingRewardsIfNecessary() {
	testcases := []struct {
	...
	}{
		{
			...
			amount: sdk.Coins{sdk.Coin{Denom: constants.BaseDenom, Amount: sdk.NewInt(1000)}},
			expErr: false,
			postCheck: func(addr sdk.AccAddress) {
		...
		},
		{
			...
			amount: sdk.Coins{sdk.Coin{Denom: constants.BaseDenom, Amount: sdk.NewInt(2e14)}},
			expErr: false,
			postCheck: func(addr sdk.AccAddress) {
				...
				case balance.Amount.Equal(sdk.NewInt(2e14)):
					suite.Require().Equal(
						sdk.DecCoins{sdk.DecCoin{Denom: constants.BaseDenom, Amount: sdk.NewDec(1e14)}},
						rewards,
		...
		},
		{
			...
			postCheck: func(addr sdk.AccAddress) {
				...
				suite.Require().NoError(err, "failed to query delegation total rewards")
				suite.Require().Equal(
					sdk.DecCoins{sdk.DecCoin{Denom: constants.BaseDenom, Amount: sdk.NewDec(1e18)}},
					rewards,
		...
		},
		{
			...
			malleate: func(addr sdk.AccAddress) {},
			amount:   sdk.Coins{sdk.Coin{Denom: constants.BaseDenom, Amount: sdk.ZeroInt()}},
			expErr:   false,
		},
		{
			name:     "pass - zero amount to be claimed",
